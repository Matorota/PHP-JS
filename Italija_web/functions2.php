...
<p><input type="hidden" name="Id" value="<?php echo $e['Id']; ?>">
<input type="submit" value="Atnaujinti" name="Keisti"></p>
</form>
<?php 
}
...
function trinti($Id) {
		...
		$conn = mysqli_connect('localhost','root','','italija') or die ("neveikia");
		$rez2 =  "delete from straipsniai where Id=".$Id."";
		mysqli_query($conn,$rez2) or die ("kazkas negerai cia");
}
function irasyti($pav, $st, $t, $nuotrauka, $data) 
{
		$conn = mysqli_connect('localhost','root','','italija');
		// sita vieta neveikia insert vieta i mysql  nesuprantu kodel
		mysqli_query($conn, "insert into straipsniai values(null,'".$pav."','Taip','".$st."','".$t."','".$nuotrauka."','".$data."', 0, 0)");
	}
